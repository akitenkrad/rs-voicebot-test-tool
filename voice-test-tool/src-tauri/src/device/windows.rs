//! Windows virtual device manager implementation.
//!
//! On Windows, virtual audio devices cannot be created programmatically.
//! Instead, this module detects pre-installed virtual audio drivers
//! such as VB-Audio Virtual Cable or VoiceMeeter via `cpal` device enumeration.
...
/// Known virtual audio device name patterns on Windows.
const VIRTUAL_DEVICE_PATTERNS: &[&str] = &["CABLE", "VB-Audio", "VoiceMeeter", "Virtual Cable"];

/// Windows device manager that detects VB-Cable / VoiceMeeter.
...
impl WindowsDeviceManager {
    pub fn new() -> Self {
        tracing::info!("Initializing Windows device manager");
        Self {
...
impl VirtualDeviceManager for WindowsDeviceManager {
    fn create_device(&mut self, name: &str) -> Result<DeviceInfo, DeviceError> {
        tracing::info!(name = %name, "Attempting to find virtual audio device on Windows");

        // On Windows we cannot create virtual devices programmatically.
        // We search for a pre-installed one (VB-Cable / VoiceMeeter).
        ...
        Err(DeviceError::CreationFailed(
            "No virtual audio device found on Windows. \
             Please install VB-Audio Virtual Cable (https://vb-audio.com/Cable/) \
             or VoiceMeeter (https://vb-audio.com/Voicemeeter/) to use virtual audio routing."
                .to_string(),
        ))
    }

    fn destroy_device(&mut self) -> Result<(), DeviceError> {
        // On Windows, virtual devices are system-level drivers.
